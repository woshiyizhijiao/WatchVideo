...
public final class SpanUtils {
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, android.text.style.LineHeightSpan {
        ...
        private Bitmap uri2Bitmap(final Uri uri) {
            try {
                return MediaStore.Images.Media.getBitmap(BaseApp.getApp().getContentResolver(), uri);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private Bitmap resource2Bitmap(final int resourceId) {
            Drawable drawable = ContextCompat.getDrawable(BaseApp.getApp(), resourceId);
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(BaseApp.getApp().getResources(), b);
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = BaseApp.getApp().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(BaseApp.getApp().getResources(), bitmap);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                    if (is != null) {
                        is.close();
                    }
                ...
                try {
                    drawable = ContextCompat.getDrawable(BaseApp.getApp(), mResourceId);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                } catch (Exception e) {
